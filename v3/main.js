/* PATCH: Main-WarRoom-Detail-Nav#v4.2 ( main~warroom-logic ) */
...
const CORE_LOGIC = {
    ...
    startWarRoomGenerator() {
        ...
        STATE.warRoomInterval = setInterval(() => {
            ...
            // SIMULASI DATA LENGKAP UNTUK DETAILS
            const fakeSig = UTILS.truncate('5' + Math.random().toString(36).substring(2) + Math.random().toString(36).substring(2), 64); // Panjang
            ...
            const log = {
                ...
                // Data Detail (Tersembunyi di tabel utama, muncul di panel bawah)
                txId: fakeSig + '...',
                makerFull: fullMaker,
...
const redrawInputLine = () => {
    ...
    process.stdout.write(P + `\x1b[36m╚═══════════════════➤ \x1b[0m` + inputBuffer);
};
...
process.stdin.on('keypress', (str, key) => {
    ...
    // 6. WAR ROOM MODE
        ...
        // NAVIGASI UP/DOWN BARU
        else if (key.name === 'up') {
            if (STATE.warRoomIdx > 0) { STATE.warRoomIdx--; RENDERER.draw(); }
        }
        else if (key.name === 'down') {
            // Hitung jumlah item visual yang ada saat ini (bisa kurang dari 15 jika log sedikit)
