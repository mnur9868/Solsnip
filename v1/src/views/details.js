/* PATCH: Enhanced-Socials#v3.9 ( view~details-socials-expansion ) */
...
export const drawDetails = () => {
    ...
    // Fungsi render link Social (Warna Cyan)
    const socLink = (i, l, u) => printLine(` ${i} ${C.C}${UTILS.padRight(l, 12)}${C.R}: ${C.D}${UTILS.truncate(u, W - 20)}${C.R}`);
    
    // 1. Render Websites
    if (row.Websites && row.Websites.length > 0) {
        row.Websites.forEach(w => {
            socLink("🌐", "WEBSITE", w.url);
        });
    }

    // 2. Render Socials
    if (row.Socials && row.Socials.length > 0) {
        row.Socials.forEach(s => {
            let icon = "💬";
            let label = s.type.toUpperCase();
            if (s.type === 'twitter') { icon = "🐦"; label = "TWITTER"; } 
            if (s.type === 'telegram') { icon = "✈️"; label = "TELEGRAM"; }
            if (s.type === 'discord') { icon = "👾"; label = "DISCORD"; }
            
            socLink(icon, label, s.url);
        });
    }
    
    // Garis pemisah tipis jika ada socials
    if ((row.Websites?.length || 0) + (row.Socials?.length || 0) > 0) {
        printLine(`${C.D}${"-".repeat(W-4)}${C.R}`);
    }

    // 3. Render Trading Terminals (Warna Merah/Standar)
    ...
    // Footer Centered (Dari fix sebelumnya)
